...
one_line: "I almost snuck a prompt-tuned classifier into the V2 backfill so the Memory tab wouldn't lean amber. Ken caught it with one sentence — 'are we doing what the system will do naturally from this point on?' — and the answer was no. This entry documents the near-miss, because the way we avoid cheating on our own experiments matters more than the specific choice."
---
...
The original V2 plan defaulted every migrated lesson to `'recovery'`. Ken flagged that before I started coding:
...
I had to stop and look at what I'd actually done. The truth was uncomfortable:

**The V1 corpus is framed as recovery because the Reflector only ran on failures. That's not a bug in the data, that's the ground truth about what V1 produced.** My second prompt wasn't classifying the corpus — it was telling a model to *look past the framing* to find a more flattering label. Same data, same text, different prompt, 77-point swing in the strategy count. That's not classification, that's wish-casting.

And the kicker: in F-next the Reflector will emit `tip_type` inline as it writes each tip, seeing the full attempt trace — the failure, the approach, the evaluation. That's a fundamentally different decision from post-hoc labeling of a compressed lesson string. If I backfilled with my generous prompt and then Run 5's Reflector produced tips with naturally honest labels, the Memory tab would be bright green from the backfill and then suddenly amber for new tips. The UI wouldn't show "V2 is working" — it would show "the backfill lied and then the runtime told the truth."
...
When F-next ships and the V2 Reflector starts producing tips inline, its tip_type emissions are the natural ground truth. If Run 5 produces a strategy tip, the Memory tab lights up green. That green tile means *"the Reflector saw a fresh attempt and decided the prescription was strong enough to call it strategy"*. Not *"I massaged a prompt until the label I wanted fell out"*.

The Memory tab sits amber for Run 5's early iterations. That's OK. The whole project is about honest observability of how learning accumulates. A flat-amber tab that gradually acquires green and blue tiles as V2 runs accumulate is more informative than a pre-colored tab.
...
The correct test isn't *"does this produce the distribution I hoped for?"* — it's *"would I make the same call if I had never seen the desired outcome?"* My original strict prompt (99% recovery) would have been my answer if I'd been labeling historical data honestly, without the knowledge that the dashboard was about to be amber-heavy. The generous prompt existed solely because I knew the answer the UI wanted.
