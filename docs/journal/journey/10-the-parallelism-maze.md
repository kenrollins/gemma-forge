...
one_line: "We thought picking a parallelism strategy would be a configuration choice. Instead, every path we tried was blocked by a different constraint — model architecture, quantization format, kernel compatibility, or vLLM's implementation — until only one option remained."
---
...
We spent real time exploring multi-model parallelism strategies,
convinced we could get 4-5x throughput with the right combination
of tensor parallelism and pipeline parallelism — and the ecosystem
blocked us at every turn until we landed on the one configuration
that actually worked.

## Why we thought this would be easy

Coming out of the model architecture revision
([entry 08](08-model-architecture-revision.md)) and the Nemotron
experiment ([entry 09](09-the-nemotron-experiment.md)), we had
what looked like a clean plan: Gemma 4 31B for the Architect and
Worker (strong reasoning), Nemotron 30B for the Auditor (fast
evaluation). Two models, two parallelism strategies, four GPUs.
The textbook answer.

The theory was compelling. On our L4s with PCIe Gen4 (~32 GB/s,
no NVLink), the parallelism strategy should matter enormously:
...
- **Tensor Parallelism (TP)** splits weight matrices across GPUs.
  ...
  over PCIe. That's the dominant bottleneck — we measured ~14
...
We tested PP=2 for Gemma 4. Crashed immediately with an
...
Nemotron is an MoE (Mixture of Experts), and we knew PP was the
right theoretical fit for MoE architectures. We started with PP=2
and it worked — Nemotron loaded, served tool calls, the Auditor
role functioned. Good.

Then we got curious: would TP=2 be faster? MoE models should
support either strategy. So we tried it. Crashed immediately
...
So we went back to PP=2, which was working fine. But now we'd
learned something valuable: PP wasn't just our preference, it was
the *only* option for Nemotron NVFP4. And the numbers were
impressive:
...
But as we documented in [entry 09](09-the-nemotron-experiment.md),
the multi-model approach introduced complexity that didn't justify
the performance gain. More importantly, we realized the Auditor
wasn't really an LLM role at all — it was a deterministic
evaluation that should be done by the harness, not a model
([entry 09](09-the-nemotron-experiment.md) has the full story
of that reframe). The Nemotron instance was shut down.
...
like wasted effort when we eventually converged on single-model
...
On our stack (vLLM 0.19.0, NVFP4, L4s without NVLink), dense
...
specific paths we can see as of April 2026:
