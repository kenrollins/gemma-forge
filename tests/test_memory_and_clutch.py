"""Property tests for v5: SQLite memory store, cross-run learning, and the clutch.
...
Tests verify:
  ...
  - SQLite WAL concurrent access safety
"""
...
from gemma_forge.harness.clutch import Clutch, ClutchConfig, ClutchState
...
def _populate_two_runs(store: SQLiteMemoryStore):
    """Helper: populate two runs with mixed outcomes for testing."""
    # Run 1: 3 easy items (authentication), 2 hard items (integrity)
    ...
    # Run 2: same categories, authentication still easy, integrity still hard
    ...
    store.save_lesson("authentication", "PAM rules respond well to authselect", r2, "auth_4")  # duplicate → weight boost
...
class TestCrossRunLearning:
    ...
    def test_property_duplicate_lessons_gain_weight(self, mem_store):
        ...
        # Saved twice (run 1 and run 2) → weight boosted above default 0.5
    ...
    def test_property_lessons_filterable_by_weight(self, mem_store):
        ...
        mem_store.save_lesson("cat_a", "strong lesson", run_id, "r2")  # boost
        mem_store.save_lesson("cat_a", "strong lesson", run_id, "r3")  # boost again
        mem_store.save_lesson("cat_a", "weak lesson", run_id, "r4")    # single, default weight
...
class TestDifficultyModel:
    def test_property_stats_aggregate_across_runs(self, mem_store):
        ...
        assert auth.completed == 4  # 3 from run 1 + 1 from run 2
        assert auth.success_rate > 0.9

        aide = cats["integrity-monitoring"]
        assert aide.escalated == 3  # 2 from run 1 + 1 from run 2
...
class TestClutchDecisions:
    ...
    def test_property_max_workers_respects_ceiling(self, mem_store):
        ...
        clutch.initialize()
        # Even easy categories cap at max_workers
...
class TestClutchBatchSelection:
    def test_property_batch_from_easy_category(self, mem_store):
        ...
        assert len(batch) == 3  # easy → max parallel
    ...
    def test_property_batch_from_hard_category(self, mem_store):
        ...
        batch = clutch.select_batch(graph)
        # Hard category → serial, but batch size is driven by max available
        # The clutch recommends 1 for hard, but select_batch takes max across ready items
        assert len(batch) >= 1
    ...
    def test_property_batch_respects_resource_conflicts(self, mem_store):
        ...
        graph.add_items([
            ...
            WorkItem(id="a2", title="A2", category="authentication",
                     resources=["/etc/pam.d/system-auth"]),  # conflicts with a1
            WorkItem(id="a3", title="A3", category="authentication",
                     resources=["/etc/ssh/sshd_config"]),     # no conflict
        ])
        graph.mark_active("a1")
        active_res = graph.get_active_resources()
        batch = clutch.select_batch(graph, active_resources=active_res)
        # a2 conflicts with active a1, so only a3 is available
