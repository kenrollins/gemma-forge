...
- [**Gemma 4 12B Data-Parallel Fleet — Bake-Off Protocol**](gemma4-12b-fleet-preflight.md) — operational companion to [ADR-0024](../../adr/0024-gemma4-12b-data-parallel-fleet-evaluation.md). The new 12B Dense fits on a single L4, which removes the PCIe all-reduce penalty that caps the 31B TP=4 engine and opens up to four independent data-parallel instances. A pre-registered 4-arm bake-off (31B control + three 12B configs) that de-confounds model size, quantization, and parallelism, with a falsifiable Tier-3 gate (STIG rules fixed per hour at acceptable Architect quality) built to dodge the Nemotron hardware-first trap.
