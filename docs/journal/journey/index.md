...
- [**38. MTP Bought 6.9 Hours and Cost 8 Rules**](38-mtp-in-practice.md) — Run 7 finished in 12.20h vs Run 6's 19.08h (1.57× faster) on the same starting state, same memory, same model — the only change was vLLM 0.21.0 + Gemma 4 MTP speculative decoding (`google/gemma-4-31B-it-assistant`, `num_speculative_tokens=2`). Per-role median tok/s held 25.9-29.0 across all 12 hours; 73% MTP acceptance aggregate; 2.46 effective tokens per decode step. The wall-clock economics are decisive. The fix rate dropped 8 rules (153 → 145) with cryptography collapsing 83% → 29% on the same starting state — two open hypotheses (drafter randomness, libvirt-snapshot non-determinism), Run 8 narrows it.
