...
def _insert_tip(
    conn, *, text: str, tip_type: str = "recovery", source_rule_id: str | None = None
) -> int:
...
def _setup_basic_fixture(mem_store):
    ...
    with mem_store._pool.connection() as conn:
        ...
        for v in (0.8, 0.8, 0.8):
            _insert_retrieval(
                conn, tip_id=tip_good, run_id=run_id, rule_id="rule_good", outcome_value=v
            )
        # 3 bad retrievals
        for v in (0.0, 0.1, 0.2):
            _insert_retrieval(
                conn, tip_id=tip_bad, run_id=run_id, rule_id="rule_bad", outcome_value=v
            )
        # 1 thin retrieval
        _insert_retrieval(
            conn, tip_id=tip_thin, run_id=run_id, rule_id="rule_thin", outcome_value=0.0
        )
...
def test_threshold_tuning(mem_store):
    pool, _tids = _setup_basic_fixture(mem_store)
...
def test_confidence_scales_utility(mem_store):
    ...
    with mem_store._pool.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO runs (id, skill, started_at) VALUES (%s, 'stig', now())", (run_id,)
            )
