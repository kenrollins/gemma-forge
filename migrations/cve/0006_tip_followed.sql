-- migrations/cve/0006_tip_followed.sql
--
-- Parity with stig.0006_tip_followed. See that file for the
-- architectural reasoning; this migration mirrors the schema
-- additions onto the CVE skill's tip_retrievals table so the
-- attribution signal lands in lockstep when CVE's next run runs.
--
-- Idempotent.
...
COMMENT ON COLUMN tip_retrievals.tip_followed_llm IS
  ...
  'for full semantics and skills/cve-response/prompts/tip_follow_judge.md '
  'for the CVE-specific judge prompt.';
