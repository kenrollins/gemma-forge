...
type: journey
title: "Journey: The Overnight Run That Taught Me More Than a Successful One Would Have"
date: 2026-04-11
...
one_line: "I ran the time-budgeted reflexion loop for 10 hours against 270 STIG rules, fixed 2, escalated 26, and discovered four architectural flaws — the most important of which was that the Worker was silently running a 15-deep retry loop inside what the harness thought was a single attempt."
---

# Journey: The Overnight Run That Taught Me More Than a Successful One Would Have

## The story in one sentence
I ran the time-budgeted Ralph loop for 10 hours against 270 STIG rules, fixed
2, escalated 26, and discovered four architectural flaws that couldn't have
been seen any other way — the most important of which was that the Worker was
silently running a 15-deep retry loop *inside* what the harness thought was a
single attempt.
...
I added a plateau-detection metric that compared the first sentences of
...
novel because the exact string didn't match. What was needed was **semantic
similarity** (embedding cosine or fuzzy match), not string equality.
...
1. **Non-tty sudo**: the SSH tool uses `asyncssh` with a non-tty session. Rules
...
halt the run entirely and alert the operator. The loop did neither. It just
kept grinding.
...
The entire reflexion architecture depends on the assumption that each
...
- **The "229 total attempts" count is a massive undercount.** The real number
...
- **A single rule's "attempts" log entries don't match the LLM's experience.**
  The event stream says "attempt 6 on rule X", but from the Worker's
...
This is a harness bug, not a model quirk. The prompt told the Worker to "call
apply_fix" and didn't constrain it to a single call, so the model filled in
the gap with its default retry instinct.
...
Even after fix #1, a deterministic token budget for each prompt is
required. Before each `_run_agent_turn` call, assemble the prompt pieces in
...
Is it the non-tty SSH session (the SSH tool's fault)? Or is it an early fix
...
## What this run taught me that a successful run would not have

1. **The Worker's internal retry loop is invisible in the logs.** Without
   catching this, v2 of the reflexion loop would have shipped to a demo
   with 80-90% of the Worker's actual behavior happening outside the
   instrumented boundary.

2. **The context bomb isn't where I thought.** My pre-run hypothesis was
...
3. **The Reflector is smart enough to diagnose correctly.** It said "STOP,
   ...
   times. A smarter Reflector isn't the fix. An Architect that listens to
   it is.

4. **Plateau detection needs semantic similarity.** The naive string match
   detector saw 0% plateau when the real rate was probably 60-80%. This was
   a false confidence signal that made the loop look "working" when it was
   stuck.
...
6. **Failure runs are more valuable than success runs for architecture work.**
   Two successful AIDE install fixes would have told me nothing. 26 failures
   taught me everything needed for v3 of the loop.
